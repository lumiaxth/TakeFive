...
- **暂停机制**：手动开关——暂停计时后，插件图标显示"停"标记，期间不计时、不限额（黑名单除外），随时可恢复。
...
- **番茄钟**：开启后按"专注 / 休息"周期倒计时（默认 25 / 5 分钟），专注阶段切换到白名单以外的标签页时提醒；白名单内的网站可正常切换。
...
- **设置限制与黑名单**：右键图标「选项」（或设置页入口），管理限额规则、黑名单、图标角标显示类型、连续使用提醒、番茄钟（时长与白名单）、暂停/恢复。
- **被阻断时**：达到限额的域名会跳转到阻断页，可点击「暂停并继续访问」暂停计时后继续；黑名单域名不提供放行入口。
